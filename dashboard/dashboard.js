...
function getHerbsCount(herbsObj) {
    ...
    return Object.values(herbsObj).filter(Boolean).length;
}
...
function updateStats(herbsObj) {
    document.getElementById('herbsMasteredCount').textContent = formatHerbsStat(herbsObj);
}
...
const herbsCard = document.getElementById('herbsMasteredCard');
herbsCard.addEventListener('click', () => showHerbsModal(currentUserData.herbsMastered || {}));
